...
for index, row in output.iterrows():
  ...
  sesso = row["sesso"]
  cols_to_update = ['sesso', 'comune_residenza']
  values = (" ",dataProvincia.loc[dataProvincia.comune==comune,["provincia"]].values[0][0])
 
  data_df.loc[(data_df.anno_nascita==anno) &
    (data_df.comune_residenza==comune) &
    (data_df.sesso==sesso), cols_to_update]= values

  
  
  i=i+1

data_df.to_csv(file_name+"ProvincesSingletonGenderSingleton.csv")
