...
import numpy as np
import os
from abstract_PrivacyChecker import AbstractPrivacyChecker

class PrivacyChecker(AbstractPrivacyChecker):
	def privacychecker(file_path):
		def start_evaluation(file_path):
			...
			output_file.write("Best quasi-identifier: " + "\n")
			for subarray in temp_result['quasi_identifiers']:
				output_file.write(",".join(subarray)+"\n")
		...
		def get_quasi_identifiers(stats):
			...
			for column_combination in stats:
				...
				elif statistic['singleton_occurrences_absolute_value']>max_absolute_value:
					quasi_identifiers = []
					quasi_identifiers.append(column_combination_as_array)
